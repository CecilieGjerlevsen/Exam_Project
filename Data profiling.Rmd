...
We now have to go through the interger and numerical values which have missing values:
```{r}
#Starting with all integer variables and imputing the median
imputed_data[,c("AMT_REQ_CREDIT_BUREAU_HOUR")][is.na(imputed_data[,c("AMT_REQ_CREDIT_BUREAU_HOUR")])] <- median(imputed_data[,c("AMT_REQ_CREDIT_BUREAU_HOUR")], na.rm = TRUE)

imputed_data[,c("AMT_REQ_CREDIT_BUREAU_DAY")][is.na(imputed_data[,c("AMT_REQ_CREDIT_BUREAU_DAY")])] <- median(imputed_data[,c("AMT_REQ_CREDIT_BUREAU_DAY")], na.rm = TRUE)

imputed_data[,c("AMT_REQ_CREDIT_BUREAU_WEEK")][is.na(imputed_data[,c("AMT_REQ_CREDIT_BUREAU_WEEK")])] <- median(imputed_data[,c("AMT_REQ_CREDIT_BUREAU_WEEK")], na.rm = TRUE)

imputed_data[,c("AMT_REQ_CREDIT_BUREAU_MON")][is.na(imputed_data[,c("AMT_REQ_CREDIT_BUREAU_MON")])] <- median(imputed_data[,c("AMT_REQ_CREDIT_BUREAU_MON")], na.rm = TRUE)

imputed_data[,c("AMT_REQ_CREDIT_BUREAU_QRT")][is.na(imputed_data[,c("AMT_REQ_CREDIT_BUREAU_QRT")])] <- median(imputed_data[,c("AMT_REQ_CREDIT_BUREAU_QRT")], na.rm = TRUE)

imputed_data[,c("CNT_FAM_MEMBERS")][is.na(imputed_data[,c("CNT_FAM_MEMBERS")])] <- median(imputed_data[,c("CNT_FAM_MEMBERS")], na.rm = TRUE)

imputed_data[,c("DEF_60_CNT_SOCIAL_CIRCLE")][is.na(imputed_data[,c("DEF_60_CNT_SOCIAL_CIRCLE")])] <- median(imputed_data[,c("DEF_60_CNT_SOCIAL_CIRCLE")], na.rm = TRUE)

imputed_data[,c("OBS_60_CNT_SOCIAL_CIRCLE")][is.na(imputed_data[,c("OBS_60_CNT_SOCIAL_CIRCLE")])] <- median(imputed_data[,c("OBS_60_CNT_SOCIAL_CIRCLE")], na.rm = TRUE)

#Now imputing the remaning numerical variables with the mean of the variable
...
Rounding the social variables since you cannot have a decimal amount in your social circle (changing from numerical to integer)
```{r}
imputed_data$OBS_60_CNT_SOCIAL_CIRCLE <- round(imputed_data$OBS_60_CNT_SOCIAL_CIRCLE)
imputed_data$DEF_60_CNT_SOCIAL_CIRCLE <- round(imputed_data$DEF_60_CNT_SOCIAL_CIRCLE)
```

FLAG own car & FLAG own realty are turned into 0, 1 answers:
```{r}
imputed_data$FLAG_OWN_CAR[imputed_data$FLAG_OWN_CAR == "Y"] <- 1
imputed_data$FLAG_OWN_CAR[imputed_data$FLAG_OWN_CAR == "N"] <- 0

imputed_data$FLAG_OWN_REALTY[imputed_data$FLAG_OWN_REALTY == "Y"] <- 1
imputed_data$FLAG_OWN_REALTY[imputed_data$FLAG_OWN_REALTY == "N"] <- 0
```

Looking at who accompanied the person:
```{r}
p1 <- ggplot(master_data,aes(x = NAME_TYPE_SUITE, fill = factor(TARGET,labels = c("No", "Yes")))) +
...
We aggregate the variable into Accompanied versus Unaccompanied:
```{r}
imputed_data$NAME_TYPE_SUITE[imputed_data$NAME_TYPE_SUITE == "Children"] <- "Accompanied"
imputed_data$NAME_TYPE_SUITE[imputed_data$NAME_TYPE_SUITE == "Family"] <- "Accompanied"
imputed_data$NAME_TYPE_SUITE[imputed_data$NAME_TYPE_SUITE == "Group of people"] <- "Accompanied"
imputed_data$NAME_TYPE_SUITE[imputed_data$NAME_TYPE_SUITE == "Other_A"] <- "Accompanied"
imputed_data$NAME_TYPE_SUITE[imputed_data$NAME_TYPE_SUITE == "Other_B"] <- "Accompanied"
imputed_data$NAME_TYPE_SUITE[imputed_data$NAME_TYPE_SUITE == "Spouse, partner"] <- "Accompanied"
```

Collecting variables within organization type:
```{r}
#Business entities
imputed_data$ORGANIZATION_TYPE[imputed_data$ORGANIZATION_TYPE == "Business Entity Type 1"] <- "Business Entity"
imputed_data$ORGANIZATION_TYPE[imputed_data$ORGANIZATION_TYPE == "Business Entity Type 2"] <- "Business Entity"
imputed_data$ORGANIZATION_TYPE[imputed_data$ORGANIZATION_TYPE == "Business Entity Type 3"] <- "Business Entity"

#Industry
imputed_data$ORGANIZATION_TYPE[imputed_data$ORGANIZATION_TYPE == "Industry: type 1"] <- "Industry"
imputed_data$ORGANIZATION_TYPE[imputed_data$ORGANIZATION_TYPE == "Industry: type 2"] <- "Industry"
imputed_data$ORGANIZATION_TYPE[imputed_data$ORGANIZATION_TYPE == "Industry: type 3"] <- "Industry"
imputed_data$ORGANIZATION_TYPE[imputed_data$ORGANIZATION_TYPE == "Industry: type 4"] <- "Industry"
imputed_data$ORGANIZATION_TYPE[imputed_data$ORGANIZATION_TYPE == "Industry: type 5"] <- "Industry"
imputed_data$ORGANIZATION_TYPE[imputed_data$ORGANIZATION_TYPE == "Industry: type 6"] <- "Industry"
imputed_data$ORGANIZATION_TYPE[imputed_data$ORGANIZATION_TYPE == "Industry: type 7"] <- "Industry"
imputed_data$ORGANIZATION_TYPE[imputed_data$ORGANIZATION_TYPE == "Industry: type 8"] <- "Industry"
imputed_data$ORGANIZATION_TYPE[imputed_data$ORGANIZATION_TYPE == "Industry: type 9"] <- "Industry"
imputed_data$ORGANIZATION_TYPE[imputed_data$ORGANIZATION_TYPE == "Industry: type 10"] <- "Industry"
imputed_data$ORGANIZATION_TYPE[imputed_data$ORGANIZATION_TYPE == "Industry: type 11"] <- "Industry"
imputed_data$ORGANIZATION_TYPE[imputed_data$ORGANIZATION_TYPE == "Industry: type 12"] <- "Industry"
imputed_data$ORGANIZATION_TYPE[imputed_data$ORGANIZATION_TYPE == "Industry: type 13"] <- "Industry"

#Trade
imputed_data$ORGANIZATION_TYPE[imputed_data$ORGANIZATION_TYPE == "Trade: type 1"] <- "Trade"
imputed_data$ORGANIZATION_TYPE[imputed_data$ORGANIZATION_TYPE == "Trade: type 2"] <- "Trade"
imputed_data$ORGANIZATION_TYPE[imputed_data$ORGANIZATION_TYPE == "Trade: type 3"] <- "Trade"
imputed_data$ORGANIZATION_TYPE[imputed_data$ORGANIZATION_TYPE == "Trade: type 4"] <- "Trade"
imputed_data$ORGANIZATION_TYPE[imputed_data$ORGANIZATION_TYPE == "Trade: type 5"] <- "Trade"
imputed_data$ORGANIZATION_TYPE[imputed_data$ORGANIZATION_TYPE == "Trade: type 6"] <- "Trade"
imputed_data$ORGANIZATION_TYPE[imputed_data$ORGANIZATION_TYPE == "Trade: type 7"] <- "Trade"

#Transport
imputed_data$ORGANIZATION_TYPE[imputed_data$ORGANIZATION_TYPE == "Transport: type 1"] <- "Transport"
imputed_data$ORGANIZATION_TYPE[imputed_data$ORGANIZATION_TYPE == "Transport: type 2"] <- "Transport"
imputed_data$ORGANIZATION_TYPE[imputed_data$ORGANIZATION_TYPE == "Transport: type 3"] <- "Transport"
imputed_data$ORGANIZATION_TYPE[imputed_data$ORGANIZATION_TYPE == "Transport: type 4"] <- "Transport"
```

```{r}
write.csv(imputed_data,"master_data.csv")
```
...
Thus before we try to do PCA. Lets first see if we have large set of correlated variables in or data set (imputed_data).
...
library(pander)
correlation_imputed<-pander(round(cor(imputed_data[,unlist(lapply(imputed_data, is.numeric))]),2))
correlation_imputed
...
corr_cross(imputed_data[,unlist(lapply(imputed_data, is.numeric))],#dataset (only numerical)
...
imputed_PCA <- imputed_data[,c(3:38)] # here we have removed SK_ID_CURR (ID variable), and TARGET (target, dependent variable)
