...
#Combining the individual tables
...
total %>% 
  ...
  geom_text(aes(label=Value),position = position_dodge(width = 1), vjust = -0.5, size = 3) + 
  scale_y_continuous(labels=scales::percent, limits = c(0,1)) +
  scale_fill_manual(values = c('#000099',"#66CC00","#CC0033",'#FFFF66'), 
                    labels = c("Payment difficulties (0 = No),feature(0 = No)", 
                               "Payment difficulties (0 = No), feature (1 = Yes)",
                               "Payment difficulties (1 = Yes), feature (0 = No)",
                               "Payment difficulties (1 = Yes), feature (1 = Yes)") ) + 
  labs(x = "", y = "Percentage", fill = "") +
...
```{r}
total %>% ggplot(aes(x = Variable, y = Percent, fill = factor(TARGET,labels = c("No", "Yes")))) +
...
```{r}
ggplot(master_data, aes(x = AMT_CREDIT, fill = factor(TARGET))) + 
